...
describe("SpotCard", () => {
  ...
  it("reserves the tag strip in the grid layout even with no category", () => {
    ...
    expect(styleOf(slot).height).toBe(
      typography.micro.lineHeight + spacing[1] * 2,
    );
  });
  ...
  it("reserves two lines for the title in the grid layout", () => {
    ...
    expect(styleOf(screen.getByText("Hidden Cove")).minHeight).toBe(
      typography.h2.lineHeight * 2,
    );
  });
  ...
  it("stacks the rating figures under the stars in the grid layout", () => {
    renderThemed(
      <SpotCard title="Hidden Cove" rating={4.5} reviewCount={12} />,
    );
...
describe("Input", () => {
  it("renders its label and value", () => {
    renderThemed(
      <Input label="Email" value="a@b.com" onChangeText={() => {}} />,
    );
  ...
  it("meets the minimum touch target", () => {
    ...
    expect(styleOf(screen.getByDisplayValue("")).minHeight).toBe(
      minTouchTarget,
    );
  });
  ...
  describe("password reveal", () => {
    ...
    it("starts hidden", () => {
      ...
      expect(screen.getByDisplayValue("hunter2").props.secureTextEntry).toBe(
        true,
      );
    });
    ...
    it("keeps two password fields independent", () => {
      ...
      expect(screen.getByDisplayValue("first").props.secureTextEntry).toBe(
        false,
      );
      expect(screen.getByDisplayValue("second").props.secureTextEntry).toBe(
        true,
      );
    });
    ...
    it("gives the reveal button a real touch target", () => {
      ...
      expect(styleOf(screen.getByLabelText("Show password")).minHeight).toBe(
        minTouchTarget,
      );
    });
  });
